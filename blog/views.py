...
def login_verifica_grupo(request):
    if request.user.groups.all():
        ...
        if request.user.groups.get().name == "Nupe":
            return render(request, 'blog/home_nupe.html')
...
@login_required
def cadatra_funcionario_nupe(request):
    if request.method == "POST":
        ...
        form_pessoa = PessoaForm()
        form_senha = PessoaPasswordForm()
    return render(request, 'blog/funcionarioNupeAdd.html', {'form': form, 'form_pessoa': form_pessoa, 'form_senha': form_senha})
